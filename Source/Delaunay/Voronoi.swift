...
open class Voronoi {
    ...
    open var plotBounds: Rectangle = .zero
    ...
    fileprivate func hullEdges()->[Edge] {
        return edges.filter { $0.isPartOfConvexHull() }
    }
    ...
    */
    ...
    fileprivate func fortunesAlgorithm(_ generateTriangles:Bool = false) {
        ...
        let dataBounds = sites.getSitesBounds()
        
        let sqrt_nsites = Int(sqrt(Double(sites.length + 4)))
